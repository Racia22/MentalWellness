  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      if (mountedRef.current) {
        ...
        // Don't clear localStorage here - let the axios interceptor handle 401 errors
        // This prevents race conditions where multiple hooks try to clear auth state
        if (status === 401) {
          console.warn('401 Unauthorized in useMoodLogs - letting axios interceptor handle it');
          // Just reject the error, don't clear localStorage here
        }
