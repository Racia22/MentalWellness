...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('LoginPage: Token and user verified, preparing navigation');
      
      // Small delay to ensure state is set in AuthContext
      await new Promise(resolve => setTimeout(resolve, 100));
      
      // Get role - handle both camelCase and PascalCase
      const role = user?.role || user?.userRole || user?.UserRole;
      
      console.log('LoginPage: Login successful, user role:', role);
