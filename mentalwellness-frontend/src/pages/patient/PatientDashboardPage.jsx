  ...
  useEffect(() => {
    const fetchPatientId = async () => {
      ...
      try {
        ...
        // This is especially important for newly registered patients
        ...
        if (id) {
          ...
          console.warn('Patient profile found but no patientId in response:', patientData);
        }
      } catch (error) {
        // Handle 404 specifically - patient profile might not exist yet for new registrations
        if (error.response?.status === 404) {
          console.warn('Patient profile not found for user:', user.userId, '- This is normal for newly registered patients');
          // Don't throw error or logout - just continue without patientId
          // The patient can still use the dashboard, they just won't have a patientId set
        } else {
          console.error('Failed to get patient data:', error);
          // Only log other errors, don't show toast or logout
        }
      } finally {
        setLoadingPatient(false);
      }
